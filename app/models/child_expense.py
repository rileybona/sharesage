...
class ChildExpense(db.Model):
    ...
    root_expense_id = db.Column(db.Integer, db.ForeignKey('root_expenses.id'), nullable=False)
    ...
    root_expense_child_expenses = db.relationship(
        "RootExpense",
        back_populates="child_expense_root_expenses"
    )

    # user : child_expense relationship 
    user_child_expenses = db.relationship(
        "User",
        back_populates="child_expense_users"
    )

    # child_expense : payments relationship 
    payments_child_expenses = db.relationship(
        "Payment",
        back_populates="child_expense_payments"
    )