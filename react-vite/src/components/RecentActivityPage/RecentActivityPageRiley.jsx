  ...
  let ul = (
    <ul>
      {payments.map((payment) => {
        return (
          <li key={payment.id}>
            <NavLink to={`/expenses/${payment.expense_id}`}>
              {users.length && inboundHelper2(payment, users, expenses)}
            </NavLink>
          </li>
        );
      })}
    ...
    <ul>
      {payments.map((payment) => {
        return (
          <li key={payment.id}>
            <NavLink to={`/expenses/${payment.expense_id}`}>
              {users.length && helper2(payment, users, expenses)}
            </NavLink>
          </li>
        );
      })}
...
function RecentActivityPage() {
  ...
  useEffect(() => {
    ...
    dispatch(getAllUsers()).then(() => setLoaded(true));
  }, [dispatch]);
  ...
  }, [allDone, expenseState, expenses.length, userState]);
  ...
  }, [myDone, allDone, expenses, currentUser.id, dispatch]);
    ...
    <>
      <div className="outer-div-recent-activity">
        ...
        <div className="right-activity">
          <h3>Paid to Me</h3>
          {!(payments2me.length > 0) ? (
            <p>no recent payments made to you </p>
          ) : (
            <div className="inbound-payments-div">
              {inboundPaymentHelper(
                payments2me,
                users,
                findMyExpenses(expenses, currentUser.id)
              )}
            </div>
          )}
        </div>
